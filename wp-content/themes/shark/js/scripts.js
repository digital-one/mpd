...
$('#map').gmap({
       ...
       // markerFile: 'http://92.60.114.159/~wlprintco/wp-content/themes/shark/images/marker.png',
         markerFile: $root+'/wp-content/themes/shark/images/marker.png',
        markerWidth:24,
        markerHeight:39,
        markerAnchorX:12,
        markerAnchorY:39
...
/*		$('#testimonial-slider').on('afterChange', function(event, slick, currentSlide){
  ...
  $('#testimonials a.close').show();
	...
	$('body').on('click','#testimonials a.close',function(e){
			//console.log('click')
			e.preventDefault();
			$(this).hide();
			_slider.fadeOut(100);
		...
		$('body').on('click','ul#testimonial-clients a.client',function(e){
...
showOverlay('true');
	if(location.pathname=='/' || location.pathname=='/~mpdco/') url=null
